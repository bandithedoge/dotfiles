...
(load! :nvim-window-picker
       {:lazy true
        :after #(let [window-picker (require :window-picker)]
                  (window-picker.setup {:highlights {:statusline {:unfocused {:fg _G.base00
                                                                              :bg _G.base0F}}}
                                        :show_prompt false}))})
...
(load! :neo-tree.nvim
       {:keys [(key! :<leader>t "<cmd>Neotree toggle<cr>" {:desc "File tree"})]
        :cmd :Neotree
        :before #(trigger! :nvim-window-picker)
        :after #(let [neo-tree (require :neo-tree)]
                  (neo-tree.setup {:sources [:filesystem]
