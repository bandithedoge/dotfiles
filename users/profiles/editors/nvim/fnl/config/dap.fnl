...
[(_G.use :mfussenegger/nvim-dap
         {:dependencies [(_G.use :rcarriga/nvim-dap-ui)]
          :keys [(_G.key :<localleader>dd :<cmd>DapUIToggle<cr> {:desc :Toggle})
                 (_G.key :<localleader>de :<cmd>DapUIEval<cr> {:desc :Eval})
          ...
          :config #(let [dapui (require :dapui)
                     ...
                     (set dap.listeners.after.event_exited.dapui_config
                          dapui.close)
                     (dapui.setup)
                     (command! [] :DapUIToggle `dapui.toggle)
                     (command! [] :DapUIEval `dapui.eval))})]
