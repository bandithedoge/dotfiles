...
in {
  ...
  # river {{{
  xdg.configFile."river/init" = let
    colors = color: "0x" + pkgs.lib.strings.removePrefix "#" color;
  in {
    executable = true;
    text = with rice; ''
      #!/usr/bin/env bash
  ...
  # yambar {{{
  xdg.configFile."yambar/config.yml".text = with rice; let
            ...
            "alsa" = {
              ...
              content.map = {
                ...
                values = {
                  ...
                  true = [{string = icon "婢";} {string = module_red "Muted";}];
                };
              };
            };
          }
  ...
  xdg.configFile."swaylock/config".text = let
    color = pkgs.lib.strings.removePrefix "#";
  in
    with rice; ''
